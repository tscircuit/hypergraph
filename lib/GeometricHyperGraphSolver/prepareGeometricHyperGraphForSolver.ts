...
import type { GeometricHyperGraph, GeometricRegion } from "./geometric-types"

export const prepareGeometricHyperGraphForSolver = <
  TGraph extends HyperGraph = HyperGraph,
>(
  graph: TGraph,
): GeometricHyperGraph => {
  for (const region of graph.regions) {
    const regionData = (region.d ?? {}) as GeometricRegion["d"] & {
      width?: number
      height?: number
    }
    ...
    if (!regionData.bounds || !regionData.center) {
      throw new Error(
        `Region ${region.regionId} is missing geometric bounds/center`,
      )
    }
  ...
  return graph as unknown as GeometricHyperGraph
}
